#################################################################################
#
#  -file = "bootstrap_VR09.R"   Code written March 2014, Updated Sept. 22 2016
...
#
#   -NOTE: This program applies the bootstrap method to the Vermeer and Rahmstorf (2009)
...
for(i in 1:NI.boot) {
  source("../Scripts/min_res_VR_bootstrapped.R") # function to minimize the residuals of the bootstrap simulations
...
for(n in 1:NI.boot) {
    boot.est = c(alpha.boot[n], T_0.boot[n], H_0.boot[n], b.boot[n])
    boot.fit[n, ] = VR_SL_model(boot.est, hist.temp)$sle
}
...
for(n in 1:NI.boot) {
    boot.est = c(alpha.boot[n], T_0.boot[n], H_0.boot[n], b.boot[n])
    proj.boot.sim[n, ] = VR_SL_model(boot.est, rcp85)$sle
}
...
boot.pdfT0 <- density(bootstrap_parameters[ ,2])
boot.pdfinitialval <- density(bootstrap_parameters[ ,3])
boot.pdfb <- density(bootstrap_parameters[ ,4])
...
to=hindcast_length #122
boot.med.hindcast = VR_SL_model(boot.med, hist.temp) # median fit hindcast

# Estimate model projection from parameter medians.
to=projection_length #421
boot.med.projection = VR_SL_model(boot.med, rcp85)
...
save.image(file = "Workspace/Bootstrap_VR_SLR_model_workspace.RData")
