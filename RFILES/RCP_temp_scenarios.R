...
to=length(years.rcp)
source("Scripts/searate_func.R") #searate function finds the projected rates for SLR to 2100

# Bootstrap RCP26 ---------------------------------------------------------
boot.rcp26.rate=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.rcp26.proj=mat.or.vec(N, length(years.rcp)) #(nr,nc) # fit.boot_proj is the projected SLR simulation without noise

for(n in 1:N) {
  boot.rcp26.rate[n,] = thermal(parm[n,], tempRCP26_anom)[[1]] #find projected rates for SLR to 2100
  boot.rcp26.proj[n,1] = initialvalue[n]
  for (i in from:to){
...
res.boot.26=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.slr.rcp26=res.boot.26
for(n in 1:N) {
  for(i in 2:length(years.rcp)) {
    res.boot.26[n,i] = rho[2]*res.boot.26[n,i-1] + rnorm(1,mean=0,sd=sigma[n])
  }
}
for(i in 1:N) {
...
# Bootstrap RCP45 ---------------------------------------------------------
boot.rcp45.rate=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.rcp45.proj=mat.or.vec(N, length(years.rcp)) #(nr,nc) # fit.boot_proj is the projected SLR simulation without noise

for(n in 1:N) {
  boot.rcp45.rate[n,] = thermal(parm[n,], tempRCP45_anom)[[1]] #find projected rates for SLR to 2100
  boot.rcp45.proj[n,1] = initialvalue[n]
  for (i in from:to){
...
### Calculate projected residuals with an lag-1 autocorrelation coefficient: rho[2]
res.boot.45=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.slr.rcp45=res.boot.45
for(n in 1:N) {
  for(i in 2:length(years.rcp)) {
    res.boot.45[n,i] = rho[2]*res.boot.45[n,i-1] + rnorm(1,mean=0,sd=sigma[n])
  }
}
for(i in 1:N) {
...
# Bootstrap RCP60 ---------------------------------------------------------
boot.rcp60.rate=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.rcp60.proj=mat.or.vec(N, length(years.rcp)) #(nr,nc) # fit.boot_proj is the projected SLR simulation without noise

for(n in 1:N) {
  boot.rcp60.rate[n,] = thermal(parm[n,], tempRCP60_anom)[[1]] #find projected rates for SLR to 2100
  boot.rcp60.proj[n,1] = initialvalue[n]
  for (i in from:to){
...
### Calculate projected residuals with an lag-1 autocorrelation coefficient: rho[2]
res.boot.60=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.slr.rcp60=res.boot.60
for(n in 1:N) {
  for(i in 2:length(years.rcp)) {
    res.boot.60[n,i] = rho[2]*res.boot.60[n,i-1] + rnorm(1,mean=0,sd=sigma[n])
  }
}
for(i in 1:N) {
...
# Bootstrap RCP85 ---------------------------------------------------------
boot.rcp85.rate=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.rcp85.proj=mat.or.vec(N, length(years.rcp)) #(nr,nc) # fit.boot_proj is the projected SLR simulation without noise

for(n in 1:N) {
  boot.rcp85.rate[n,] = thermal(parm[n,], tempRCP85_anom)[[1]] #find projected rates for SLR to 2100
  boot.rcp85.proj[n,1] = initialvalue[n]
  for (i in from:to){
...
### Calculate projected residuals with an lag-1 autocorrelation coefficient: rho[2]
res.boot.85=mat.or.vec(N, length(years.rcp)) #(nr,nc)
boot.slr.rcp85=res.boot.85
for(n in 1:N) {
  for(i in 2:length(years.rcp)) {
    res.boot.85[n,i] = rho[2]*res.boot.85[n,i-1] + rnorm(1,mean=0,sd=sigma[n])
  }
}
for(i in 1:N) {
...
###############################################################################
# Homoskedastic RCP26 ---------------------------------------------------------
homo.rcp26.rate=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.rcp26.proj=mat.or.vec(k, length(years.rcp)) #(nr,nc) 
par=mat.or.vec(k, 2)

for(n in 1:k) {
  par[n,1]=sschain[n,1] # alpha parameter
  par[n,2]=sschain[n,2] # T0 parameter
  homo.rcp26.rate[n,] = thermal(par[n,], tempRCP26_anom)[[1]]
  homo.rcp26.proj[n,1] = sschain[n,3] # initial value in 1880
  for (i in from:to){
    homo.rcp26.proj[n,i]=homo.rcp26.proj[n,i-1]+homo.rcp26.rate[n,i-1]*timestep
  }
}

res.homo_26=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.slr.rcp26=res.homo_26
for(n in 1:k) {
  for(i in 2:length(years.rcp)) {
    res.homo_26[n,i] = sschain[n,5]*res.homo_26[n,i-1] + 
...
# Homoskedastic RCP45 ---------------------------------------------------------
homo.rcp45.rate=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.rcp45.proj=mat.or.vec(k, length(years.rcp)) #(nr,nc) 

for(n in 1:k) {
  homo.rcp45.rate[n,] = thermal(par[n,], tempRCP45_anom)[[1]]
  homo.rcp45.proj[n,1] = sschain[n,3] # initial value in 1880
  for (i in from:to){
    homo.rcp45.proj[n,i]=homo.rcp45.proj[n,i-1]+homo.rcp45.rate[n,i-1]*timestep
  }
}

res.homo_45=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.slr.rcp45=res.homo_45
for(n in 1:k) {
  for(i in 2:length(years.rcp)) {
    res.homo_45[n,i] = sschain[n,5]*res.homo_45[n,i-1] + 
      rnorm(1,mean=0,sd=sschain[n,4])
  }
}
### superimpose residuals on the projections from the SLR model ###
...
# Homoskedastic RCP60 ---------------------------------------------------------
homo.rcp60.rate=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.rcp60.proj=mat.or.vec(k, length(years.rcp)) #(nr,nc) 

for(n in 1:k) {
  homo.rcp60.rate[n,] = thermal(par[n,], tempRCP60_anom)[[1]]
  homo.rcp60.proj[n,1] = sschain[n,3] # initial value in 1880
  for (i in from:to){
    homo.rcp60.proj[n,i]=homo.rcp60.proj[n,i-1]+homo.rcp60.rate[n,i-1]*timestep
  }
}

res.homo_60=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.slr.rcp60=res.homo_60
for(n in 1:k) {
  for(i in 2:length(years.rcp)) {
    res.homo_60[n,i] = sschain[n,5]*res.homo_60[n,i-1] + 
      rnorm(1,mean=0,sd=sschain[n,4])
  }
}
### superimpose residuals on the projections from the SLR model ###
...
# Homoskedastic RCP85 ---------------------------------------------------------
homo.rcp85.rate=mat.or.vec(k, length(years.rcp)) #(nr,nc)
homo.rcp85.proj=mat.or.vec(k, length(years.rcp)) #(nr,nc) 

for(n in 1:k) {
  homo.rcp85.rate[n,] = thermal(par[n,], tempRCP85_anom)[[1]]
  homo.rcp85.proj[n,1] = sschain[n,3] # initial value in 1880
  for (i in from:to){
...
for(n in 1:k) {
  for(i in 2:length(years.rcp)) {
    res.homo_85[n,i] = sschain[n,5]*res.homo_85[n,i-1] + 
      rnorm(1,mean=0,sd=sschain[n,4])
  }
}
### superimpose residuals on the projections from the SLR model ###
...
heter.rcp26.rate=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.rcp26.proj=mat.or.vec(h, length(years.rcp)) #(nr,nc) 
par=mat.or.vec(h, 2)

for(n in 1:h) {
  par[n,1]=ssprechain1[n,1] # alpha parameter
  par[n,2]=ssprechain1[n,2] # T0 parameter
  heter.rcp26.rate[n,] = thermal(par[n,], tempRCP26_anom)[[1]]
  heter.rcp26.proj[n,1] = ssprechain1[n,3] # initial value in 1880
  for (i in from:to){
    heter.rcp26.proj[n,i]=heter.rcp26.proj[n,i-1]+heter.rcp26.rate[n,i-1]*timestep
  }
}
res.heter_26=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.slr.rcp26=res.heter_26
for(n in 1:h) {
  for(i in 2:length(years.rcp)) {
    res.heter_26[n,i] = ssprechain1[n,5]*res.heter_26[n,i-1] + 
...
heter.rcp45.rate=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.rcp45.proj=mat.or.vec(h, length(years.rcp)) #(nr,nc) 

for(n in 1:h) {
  heter.rcp45.rate[n,] = thermal(par[n,], tempRCP45_anom)[[1]]
  heter.rcp45.proj[n,1] = ssprechain1[n,3] # initial value in 1880
  for (i in from:to){
    heter.rcp45.proj[n,i]=heter.rcp45.proj[n,i-1]+heter.rcp45.rate[n,i-1]*timestep
  }
}
res.heter_45=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.slr.rcp45=res.heter_45
for(n in 1:h) {
  for(i in 2:length(years.rcp)) {
    res.heter_45[n,i] = ssprechain1[n,5]*res.heter_45[n,i-1] + 
      rnorm(1,mean=0,sd=ssprechain1[n,4])
  }
}
### superimpose residuals on the projections from the SLR model ###
...
heter.rcp60.rate=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.rcp60.proj=mat.or.vec(h, length(years.rcp)) #(nr,nc) 

for(n in 1:h) {
  heter.rcp60.rate[n,] = thermal(par[n,], tempRCP60_anom)[[1]]
  heter.rcp60.proj[n,1] = ssprechain1[n,3] # initial value in 1880
  for (i in from:to){
    heter.rcp60.proj[n,i]=heter.rcp60.proj[n,i-1]+heter.rcp60.rate[n,i-1]*timestep
  }
}
res.heter_60=mat.or.vec(h, length(years.rcp)) #(nr,nc)
for(n in 1:h) {
  for(i in 2:length(years.rcp)) {
    res.heter_60[n,i] = ssprechain1[n,5]*res.heter_60[n,i-1] + 
      rnorm(1,mean=0,sd=ssprechain1[n,4])
  }
}
...
heter.rcp85.rate=mat.or.vec(h, length(years.rcp)) #(nr,nc)
heter.rcp85.proj=mat.or.vec(h, length(years.rcp)) #(nr,nc) 

for(n in 1:h) {
  heter.rcp85.rate[n,] = thermal(par[n,], tempRCP85_anom)[[1]]
  heter.rcp85.proj[n,1] = ssprechain1[n,3] # initial value in 1880
  for (i in from:to){
    heter.rcp85.proj[n,i]=heter.rcp85.proj[n,i-1]+heter.rcp85.rate[n,i-1]*timestep
  }
}
res.heter_85=mat.or.vec(h, length(years.rcp)) #(nr,nc)
for(n in 1:h) {
  for(i in 2:length(years.rcp)) {
    res.heter_85[n,i] = ssprechain1[n,5]*res.heter_85[n,i-1] + 
      rnorm(1,mean=0,sd=ssprechain1[n,4])
  }
}
### superimpose residuals on the projections from the SLR model ###
