#################################################################################
#
#  -file = "Grinobs_likelihood_AR.R"   Code written July 2014
...
#       sea-level rise observations as described in  Ruckert et al. (2016).
...
#   source("Grinobs_likelihood_AR.R")
#
####################################################################################
...
{ 
  ...
  model.out = grinsted_sealevel(par, hist.temp) # run the SLR model and name to output y.mod
  y.mod = model.out$sle
...
log.pri = function(p)
{
  ...
  if(in.range) {
    # Assign a gamma prior to tau.y (parameter 4)
    lpri.invtau = dgamma(tau.y, shape=shape.tau, scale=scale.tau, log=TRUE)
    lpri = 0 + lpri.invtau
  } else {
    lpri = -Inf
  }
